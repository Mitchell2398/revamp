 ...
 */
import { withErrorHandling, writeProgress, writeSuccess, writeInfo, writeNextStep, writeHintBlock } from '../utils/cli.js';
...
import { jobHandler } from '../primitives/job.handler.js';
...
async function displayJobsCore(context, jobs) {
    ...
    for (const job of jobs) {
        try {
            const jobInstance = await jobApi.getJob(job.jobId);
            const jobData = jobInstance.data;
...
async function fetchVersionsCore(context, job) {
    const jobApi = new JobApi(BASE_URLS.API, context.apiKey, context.agentId);
    const [versionsResponse, jobResponse] = await Promise.all([
        jobApi.getJobVersions(job.jobId),
        jobApi.getJob(job.jobId),
    ]);
    if (!versionsResponse.success || !versionsResponse.data) {
        console.error(`❌ Failed to fetch versions: ${versionsResponse.error?.message || 'Unknown error'}`);
        return null;
    }
    const versions = versionsResponse.data.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    const activeVersionId = jobResponse.data?.activeVersionId;
    return { versions, activeVersionId };
}
/**
 * Displays versions for a job.
 */
function displayVersionsCore(job, versions, activeVersionId) {
    console.log("\n" + "=".repeat(60));
...
async function deployVersionCore(context, job, version) {
    ...
    writeHintBlock({
        headline: 'Job is live. Trigger it, then inspect:',
        lines: [
            { label: 'Trigger now:', command: `lua jobs trigger --job-name ${job.name}` },
            { label: 'Inspect:', command: buildLogsCommand({ primitiveType: 'job', name: job.name, limit: 5 }) },
        ],
        when: 'success',
    });
    return true;
...
async function triggerJobCore(context, job) {
    ...
    if (response.success && response.data) {
        ...
        writeNextStep({
            command: buildLogsCommand({ primitiveType: 'job', name: job.name, limit: 5 }),
            rationale: 'to see execution output',
            when: 'success',
        });
        return true;
    }
...
/**
 * Fetches live status for each config job from the server and merges it in.
 * Falls back to the original config job if the server request fails or the job is not found.
 */
async function enrichJobsWithServerStatus(context, configJobs) {
    const jobApi = new JobApi(BASE_URLS.API, context.apiKey, context.agentId);
    const serverResponse = await jobApi.getJobs();
    if (!serverResponse.success || !serverResponse.data?.jobs)
        return configJobs;
    const serverMap = new Map(serverResponse.data.jobs.map((j) => [j.id, j]));
    return configJobs.map((job) => {
        const serverJob = serverMap.get(job.jobId);
        return serverJob ? { ...job, status: serverJob.status, active: serverJob.active } : job;
    });
}
/**
 * Prompts user to select a job from a list.
 */
async function promptJobSelection(jobs, message, options) {
    const jobAnswer = await safePrompt([
        {
            ...
            choices: jobs.map((job) => {
                const statusBadge = options?.showStatus && job.status ? ` [${job.status}]` : '';
                return {
                    name: `${job.name} (${job.jobId})${statusBadge}`,
                    value: job
                };
            })
        }
...
async function executeNonInteractive(context, config, action, options) {
    ...
    // For delete and trigger actions, also check server jobs (for orphaned jobs not in local YAML)
    if (!selectedJob && (normalizedAction === 'delete' || normalizedAction === 'trigger')) {
        ...
        if (serverResponse.success && serverResponse.data?.jobs) {
            ...
            if (serverJob) {
                const activeVersion = serverJob.activeVersion ?? serverJob.versions.find((v) => v.active) ?? serverJob.versions[0];
                selectedJob = {
                    name: serverJob.name,
                    jobId: serverJob.id,
    ...
    switch (normalizedAction) {
        case 'versions': {
            const vData = await fetchVersionsCore(context, selectedJob);
            if (!vData)
                throw new Error('Failed to fetch job versions');
            ...
            displayVersionsCore(selectedJob, vData.versions, vData.activeVersionId);
            break;
        }
        case 'deploy': {
            ...
            const dData = await fetchVersionsCore(context, selectedJob);
            if (!dData)
                throw new Error('Failed to fetch job versions');
            ...
            const resolvedVersion = resolveVersion(dData.versions, options.jobVersion);
            if (!resolvedVersion)
                throw new Error('Failed to resolve job version');
            const success = await deployVersionCore(context, selectedJob, resolvedVersion);
            if (!success)
                throw new Error('Failed to deploy job version');
            break;
        }
        case 'activate': {
...
async function viewJobVersionsInteractive(context, config) {
    ...
    const viewData = await fetchVersionsCore(context, selectedJob);
    if (!viewData) {
        await safePrompt([{ type: 'input', name: 'continue', message: 'Press Enter to continue...' }]);
        return;
    }
    if (viewData.versions.length === 0) {
        ...
        displayVersionsCore(selectedJob, viewData.versions, viewData.activeVersionId);
        console.log();
    }
    await safePrompt([{ type: 'input', name: 'continue', message: 'Press Enter to continue...' }]);
}
async function deployJobVersionInteractive(context, config) {
    ...
    if (!selectedJob)
        return;
    writeProgress(`🔄 Loading versions for ${selectedJob.name}...`);
    const deployData = await fetchVersionsCore(context, selectedJob);
    if (!deployData) {
        await safePrompt([{ type: 'input', name: 'continue', message: 'Press Enter to continue...' }]);
        return;
    }
    if (deployData.versions.length === 0) {
        console.log(`\nℹ️  No versions found for ${selectedJob.name}.\n`);
        console.log("💡 Push a version first using 'lua push job'.\n");
        await safePrompt([{ type: 'input', name: 'continue', message: 'Press Enter to continue...' }]);
        return;
    }
    const selectedVersion = await promptVersionSelection(deployData.versions, deployData.activeVersionId);
    if (!selectedVersion)
        return;
...
async function activateJobInteractive(context, config) {
    ...
    jobs = await enrichJobsWithServerStatus(context, jobs);
    const selectedJob = await promptJobSelection(jobs, 'Select a job to activate:', { showStatus: true });
    if (!selectedJob)
        return;
...
async function deactivateJobInteractive(context, config) {
    ...
    jobs = await enrichJobsWithServerStatus(context, jobs);
    const selectedJob = await promptJobSelection(jobs, 'Select a job to deactivate:', { showStatus: true });
    if (!selectedJob)
        return;
    const confirmAnswer = await safePrompt([
        {
...
async function triggerJobInteractive(context, config) {
    const { merged, orphanIds, serverFailed } = await jobHandler.fetchMergedForInteraction(context.apiKey, context.agentId, config);
    if (serverFailed) {
    ...
    const answer = await safePrompt([
        {
            ...
            choices: merged.map((item) => ({
                name: orphanIds.has(item.jobId)
                    ? `${item.name} (${item.jobId}) [server only]`
                    : `${item.name} (${item.jobId})`,
                value: item,
            })),
        },
    ]);
    const selectedJob = answer?.selected ?? null;
    if (!selectedJob)
        return;
    ...
    const answer = await safePrompt([
        {
            type: 'list',
            name: 'selected',
            message: 'Select a job to delete:',
            choices: merged.map((item) => ({
                name: orphanIds.has(item.jobId)
                    ? `${item.name} (${item.jobId}) [server only]`
                    : `${item.name} (${item.jobId})`,
