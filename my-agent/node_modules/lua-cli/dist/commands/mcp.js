 ...
 */
import { withErrorHandling, writeProgress, writeSuccess, writeInfo, writeHintBlock, writeNextStep } from '../utils/cli.js';
...
export async function activateServerCore(context, server) {
    ...
    try {
        ...
        if (result.success) {
            ...
            writeHintBlock({
                headline: 'MCP server active. Test it via chat:',
                lines: [
                    { label: 'Chat test:', command: 'lua chat -e production -m "use your MCP tool"' },
                    { label: 'Inspect:', command: buildLogsCommand({ primitiveType: 'mcp', limit: 10 }) },
                ],
                when: 'success',
            });
            return true;
...
export async function deactivateServerCore(context, server) {
    ...
    try {
        ...
        if (result.success) {
            ...
            writeNextStep({
                command: `lua mcp activate --server-name ${server.name}`,
                rationale: 'to re-enable it',
                when: 'success',
            });
            return true;
        }
