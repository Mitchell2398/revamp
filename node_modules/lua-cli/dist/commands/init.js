...
import { promptAgentChoice, promptOrganizationSelection, promptAgentSelection, promptMetadataCollection, promptAgentName, promptModelSelection, displayPersonaInstructions, } from "../utils/init-prompts.js";
...
export async function initCommand(options = {}) {
    ...
    return withErrorHandling(async () => {
        ...
        // Step 4: Execute based on mode — select or create agent
        ...
        // Fetch server state directly rather than agentHandler.checkDrift — drift detection
        ...
        if (sourcesRestored && !isNewAgent) {
            try {
                ...
                writeSuccess('  ✅ Agent configuration synced');
            }
            catch (error) {
                writeInfo('  ⚠ Failed to sync agent config — run "lua sync" manually');
            }
        }
        ...
        // New agents already have the model set in the create request — no separate PATCH needed.
        const userChoseModel = selectedModel && selectedModel !== serverModel;
        if (userChoseModel && !isNewAgent) {
            try {
                writeProgress(`🤖 Setting model: ${selectedModel}...`);
                await updateAgentModel(apiKey, selectedAgent.agentId, selectedModel);
                writeProgress(`✅ Model set: ${selectedModel}`);
            }
            catch (error) {
                writeInfo(`⚠️  Could not set model on server — you can push it later with \`lua push agent\``);
            }
        }
...
async function createNewAgentNonInteractive(apiKey, userData, agentName, orgId, orgName, promoCode, model) {
    ...
    // Create the agent (model included in request — no separate PATCH needed)
