...
async function listSkillNonInteractive(marketplaceApi, config, apiKey, options) {
    ...
    if (existingSkill?.listed) {
        console.error(`❌ Skill "${skill.name}" is already listed as "${existingSkill.displayName}".`);
        throw new Error("Skill");
    }
    // List the skill
    writeProgress('🔄 Listing skill on marketplace...');
...
async function publishVersionNonInteractive(marketplaceApi, config, apiKey, options) {
    ...
    if (envVarsJson) {
        try {
            ...
            console.error('❌ Invalid --env-vars-json: must be valid JSON');
    ...
    writeSuccess(`✅ Version ${publishedVersion.version} published successfully!`);
...
async function updateMetadataNonInteractive(marketplaceApi, options) {
    ...
    if (!displayName) {
        console.error('❌ No update specified. Provide --display-name to update.');
        throw new Error("No update specified. Provide --display-name to update.");
    }
    writeProgress('🔄 Updating skill metadata...');
    await marketplaceApi.updateSkill(marketplaceId, { displayName });
    writeSuccess(`✅ Display name updated to "${displayName}"!`);
}
/**
...
async function unlistSkillNonInteractive(marketplaceApi, options) {
    ...
    writeProgress('🔄 Unlisting skill...');
    await marketplaceApi.unlistSkill(marketplaceId);
    writeSuccess('✅ Skill unlisted from marketplace.');
    writeInfo('💡 You can re-list it anytime using the list action.');
}
/**
...
async function unpublishVersionNonInteractive(marketplaceApi, options) {
    ...
    writeProgress('🔄 Unpublishing version...');
    await marketplaceApi.unpublishVersion(marketplaceId, versionId);
    writeSuccess('✅ Version unpublished successfully.');
    writeInfo('💡 Users can no longer install this version.');
}
/**
...
async function viewMyListedSkillsNonInteractive(marketplaceApi, options) {
    ...
    if (!skills || skills.length === 0) {
        writeInfo('📦 You haven\'t listed any skills on the marketplace yet.');
        return;
    }
    if (options.json) {
    ...
    console.log(`\n📊 You have ${skills.length} skill(s) on the marketplace:\n`);
...
async function searchSkillsNonInteractive(marketplaceApi, options) {
    ...
    writeProgress(options.query ? `🔍 Searching for "${options.query}"...` : '🔍 Loading marketplace skills...');
    ...
    if (skills.length === 0) {
        writeInfo(options.query
            ? `📦 No skills found matching "${options.query}".`
            : '📦 No skills available on the marketplace.');
        return;
    }
    const totalCount = pagination?.totalCount || skills.length;
    const totalPages = pagination?.totalPages || 1;
    console.log(`\n📊 Found ${totalCount} skill(s) (Page ${page}/${totalPages}):\n`);
    for (const skill of skills) {
...
async function viewSkillNonInteractive(marketplaceApi, options) {
    ...
    writeProgress('🔄 Loading skill details...');
...
async function installSkillNonInteractive(marketplaceApi, config, options) {
    ...
    writeProgress('🔄 Installing skill...');
    ...
    writeSuccess('✅ Skill installed successfully!');
...
async function updateInstalledSkillNonInteractive(marketplaceApi, config, apiKey, options) {
    ...
    writeProgress('🔄 Loading installed skills...');
    ...
    const installedSkill = installedSkills?.find(s => s.name === skillName ||
        s.title === skillName);
    if (!installedSkill) {
        console.error(`❌ Installed skill not found: ${skillName}`);
    ...
    if (!installedSkill.marketplaceSkillId) {
        console.error('❌ Skill is missing marketplace skill ID.');
    ...
    if (!updatePayload.versionId && !updatePayload.envVars) {
        console.error('❌ No update specified. Provide --version-id or --env-vars.');
        throw new Error("No update specified. Provide --version-id or --env-vars.");
    }
    writeProgress('🔄 Updating installed skill...');
    await marketplaceApi.updateInstallation(installedSkill.marketplaceSkillId, agentId, updatePayload);
    writeSuccess(`✅ "${installedSkill.title || installedSkill.name}" updated successfully!`);
}
/**
...
async function uninstallSkillNonInteractive(marketplaceApi, config, options) {
    ...
    if (!force) {
        console.error(`❌ Use --force to confirm uninstalling "${installedSkill.title || installedSkill.name}"`);
        throw new Error("Use --force to confirm uninstalling");
    }
    writeProgress('🔄 Uninstalling skill...');
    await marketplaceApi.uninstallSkill(installedSkill.marketplaceSkillId, agentId);
    writeSuccess(`✅ "${installedSkill.title || installedSkill.name}" uninstalled successfully!`);
}
/**
...
async function listInstalledSkillsNonInteractive(marketplaceApi, config, options) {
    ...
    writeProgress('🔄 Loading installed marketplace skills...');
    const skills = await marketplaceApi.getInstalledSkills(agentId);
    if (!skills || skills.length === 0) {
        writeInfo('📦 No marketplace skills installed on this agent.');
        return;
    }
    if (options.json) {
        console.log(JSON.stringify(skills, null, 2));
    ...
    console.log(`\n📊 You have ${skills.length} marketplace skill(s) installed:\n`);
    for (const skill of skills) {
...
async function browseAndSelectSkill(marketplaceApi, purpose, publishedVersionsOnly) {
    try {
        ...
        while (true) {
            ...
            console.log(`\n📊 Found ${totalCount} skill${totalCount === 1 ? "" : "s"}${pageInfo}:\n`);
